...
{
    public async Task<TimetableEntity?> GetByIdAsync(int id) => await context.Timetables.Include(t => t.Days)
        .Include(t => t.Periods).Include(t => t.SelectedSubjects).FirstOrDefaultAsync(t => t.Id == id);

    public async Task<List<TimetableEntity>> GetAllAsync() => await context.Timetables.ToListAsync();
    public async Task AddAsync(TimetableEntity entity) => await context.Timetables.AddAsync(entity);
}
